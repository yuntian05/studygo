...
func parseFileInfoFrom(res *http.Response) string {
	contentDisposition := res.Header.Get("Content-Disposition")
	if contentDisposition != "" {
		_, param, err := mime.ParseMediaType(contentDisposition)
...
func (d *FileDownloader)downloadPart(c filePart) error  {
	...
	if res.StatusCode > 299 {
		return errors.New(fmt.Sprintf("服务器错误状态码: %v", res.StatusCode))
	}
...
func (d *FileDownloader) head() (int, error) {
	...
	if res.StatusCode > 299 {
		return 0, errors.New(fmt.Sprintf("Can not process, response is %v", res.StatusCode))
	}
	...
	d.outputFileName = parseFileInfoFrom(res)
